...
def tests_to_run(forwarder):
    from tests import BasicTest, SackBasicTest, RandomDropTest, SackRandomDropTest, DropAllTest
    from tests import WrongOrderTest, SackWrongOrderTest, DupMessageTest, SackDupMessageTest
    from tests import CorruptTest, StartTest, EndTest, PictureTest, VideoTest

    # BasicTest.BasicTest(forwarder, "README")
    # SackBasicTest.SackBasicTest(forwarder,"README")

    # RandomDropTest.RandomDropTest(forwarder, "README")
    # SackRandomDropTest.SackRandomDropTest(forwarder, "README")
    # DropAllTest.DropAllTest(forwarder, "README")

    # WrongOrderTest.WrongOrderTest(forwarder, "README")
    # SackWrongOrderTest.SackWrongOrderTest(forwarder, "README")

    # DupMessageTest.DupMessageTest(forwarder, "README")
    # SackDupMessageTest.SackDupMessageTest(forwarder, "README")

    # CorruptTest.CorruptTest(forwarder,"README")
    # StartTest.StartTest(forwarder,"README")
    # EndTest.EndTest(forwarder,"README")

    # PictureTest.PictureTest(forwarder,"testPic.jpg")

    # VideoTest.VideoTest(forwarder,"testVideo.mp4")
"""
...
class Forwarder(object):
    ...
    def handle_receive(self, message, address, sackMode = False):
        ...
        if self.test_state == "READY":
            if address == self.receiver_addr:           # 根据发出地址(包括端口)不同，确定packet的目标地址
        ...
        try:
            ...
            while sender.poll() is None:
                try:
                    ...
                    pass
                except Exception as e:      # 处理重复发送测试时，关闭远程连接的奇怪现象
                    # print(e)
                    pass
